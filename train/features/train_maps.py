...
from config import get_resource_path, TRIALS
...
def run_train():
    ...
    dataset = MapDataset(train_config.DATA_MAP_ROOT, TRIALS[0]["map_list"], transform=train_transform)
    ...
    model.fc = nn.Linear(in_features, train_config.NUM_CLASSES)
    ...
    # 保存模型权重
    p = get_resource_path(os.path.join("assets", "features_resnet50_map_classifier.pt"))

    torch.save({
        "model_state_dict": model.state_dict(),
        "class_names": train_config.MAP_LIST
    }, p)

    print(f"训练完成！特征库保存至: {p}")
